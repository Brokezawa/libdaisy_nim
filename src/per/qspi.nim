...
type
  ...
  QSPIDevice* {.importcpp: "daisy::QSPIHandle::Config::Device", pure.} = enum
    IS25LP080D = "daisy::QSPIHandle::Config::Device::IS25LP080D"  ## 8 Mbit (1 MB)
    IS25LP064A = "daisy::QSPIHandle::Config::Device::IS25LP064A"  ## 64 Mbit (8 MB)

  QSPIMode* {.importcpp: "daisy::QSPIHandle::Config::Mode", pure.} = enum
    MEMORY_MAPPED = "daisy::QSPIHandle::Config::Mode::MEMORY_MAPPED"  ## Read-only memory-mapped access
    INDIRECT_POLLING = "daisy::QSPIHandle::Config::Mode::INDIRECT_POLLING"  ## Full read/write/erase access

  QSPIConfig* {.importcpp: "daisy::QSPIHandle::Config", bycopy.} = object
    ## Configuration for QSPI flash interface
    device* {.importcpp: "device".}: QSPIDevice  ## Flash chip model
    mode* {.importcpp: "mode".}: QSPIMode  ## Operating mode
