...
## See Also
## --------
## - `libdaisy_system <libdaisy_system.html>`_ - Timing functions for profiling
...
when isMainModule:
  import std/strformat  # Only for examples

useDaisyModules(logger)
...
when isMainModule:
  ...
  block:
    type MyLogger = Logger[LOGGER_INTERNAL]
  ...
  block:
    ...
    let msg = &"V={voltage:.2f}V, I={current:.3f}A"
    UsbLogger.log(msg)
  ...
  block:
    ...
    let startTime = 1000'u32
    let endTime = 1523'u32
    let duration = endTime - startTime
    
    UsbLogger.log(&"Function took {duration} us")
  
  # Example 4: Conditional logging
  block:
    when defined(release):
      type AppLogger = NullLogger  # Muted in release
    else:
      type AppLogger = UsbLogger   # Active in debug
    
    AppLogger.init()
    AppLogger.log("This only appears in debug builds")
  
  # Example 5: Multiple destinations
  block:
    type
      InternalLog = Logger[LOGGER_INTERNAL]
      ExternalLog = Logger[LOGGER_EXTERNAL]
    
    InternalLog.startLog()
    ExternalLog.startLog()
    
    InternalLog.log("Internal USB")
    ExternalLog.log("External USB")
  
  # Example 6: Data structure logging
  block:
    type Point = object
      x, y: float32
    
    var pos = Point(x: 1.5, y: 2.3)
    
    UsbLogger.log(&"Position: ({pos.x:.1f}, {pos.y:.1f})")
  
  # Example 7: Loop progress
  block:
    UsbLogger.startLog()
    
    for i in 0..<10:
      UsbLogger.log(&"Processing item {i + 1}/10")
  
  # Example 8: Using the convenience alias
  block:
    UsbLogger.init()
    
    let firmwareVersion = "1.0.0"
    UsbLogger.log(&"Firmware v{firmwareVersion}")
    UsbLogger.log("Ready")
