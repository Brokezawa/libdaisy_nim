...
- Check silkscreen on board (Rev number)
- Try initializing with both device types (one will succeed)
- Rev 5+ boards have improved power supply and 8MB flash
...
- ⚠️ Erase is SLOW (~100ms per sector)
- ⚠️ Do NOT erase from audio callback!
...
- ⚠️ Must erase before writing `1` bits
- ⚠️ Write is SLOW (~1ms per page)
- ⚠️ Do NOT write from audio callback!
